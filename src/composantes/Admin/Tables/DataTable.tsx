      ...
      {searchable && (
        <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
          <input
            ...
            className="w-64 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
          />
        ...
        <thead className="bg-cyan-600 text-white">
          <tr>
            {columns.map((col) => (
              <th
                key={String(col.key)}
                className={`p-3 text-left whitespace-nowrap ${col.sortable ? "cursor-pointer select-none" : ""
                  }`}
onClick={() => {
  try {
    if (col.sortable) {
    ...
    console.error("Erreur clic en-tête :", err);
              ...
              >
                {col.label}
                {col.sortable && <ArrowUpDown className="inline w-4 h-4 ml-1" />}
              </th>
            ))}
          </tr>
        </thead>
                  ...
                  >
                    {columns.map((col) => (
                      ...
                      >
                        ...
                        {col.render
                          ? col.render(row)
                          : truncateText(String(row[col.key as keyof T] ?? ""), 60)}
                          </strong>
                      </td>
        ...
        <div className="flex justify-between items-center p-4 bg-gray-50 text-sm text-gray-600">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 rounded-lg border ${page === 1
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg border ${page === totalPages
                  ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                  : "bg-white hover:bg-gray-100"
                }`}
            >
